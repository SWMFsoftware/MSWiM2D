...
for line in lines:
    ...
    if('Start' in line):start_in = line.split(':')[-1].strip()
    if('End' in line):end_in = line.split(':')[-1].strip()
    if('Resolution' in line):interval = line.split(':')[-1].strip()
infile.close()

# Construct start and end times.
start_time = dt.datetime(int(start_in[:4]), int(start_in[5:7]), int(start_in[-2:]))
end_time = dt.datetime(int(end_in[:4]), int(end_in[5:7]), int(end_in[-2:]))

# Create target name for SPICE.
planets_list=['EARTH','MARS','JUPITER','SATURN','URANUS','NEPTUNE','PLUTO']
satellites_dict={'CASSINI':[dt.datetime(2002,4,11),dt.datetime(2008,8,8)],
                 'VOYAGER 1':[dt.datetime(1977,9,6),dt.datetime(2030,12,31)],
                 'VOYAGER 2':[dt.datetime(1977,8,21),dt.datetime(2030,12,31)],
                 'JUNO':[dt.datetime(2011,8,6),dt.datetime(2017,10,16)],
...
# r, phi = list(),list()
# for i,entry in enumerate(positions[0]):
#     r.append(np.sqrt(positions[1][i]**2+positions[0][i]**2))
#     phi.append(np.degrees(math.atan2(positions[1][i],positions[0][i])))
# print('X: ',positions[0][0],positions[0][-1])
# print('Y: ',positions[1][0],positions[1][-1])
# print('R: ',r[0],r[-1])
# print('Phi: ',phi[0],phi[-1])
plottraj = False
if(plottraj):
